...
import { useMessages } from '../data/domain/useMessages';
...
export default function Hub() {
  ...
  const handleAddMsg = async (type: 'note' | 'reminder' | 'question') => {
  ...
  const togglePin = async (id: string, currentVal: boolean) => {
    try {
      await updateMessage(id, { is_pinned: !currentVal });
    } catch (err) {
      console.error(err);
    }
  };
        ...
        <div className="flex gap-2 mb-6">
          <input 
            ...
            onKeyDown={e => {
              if (e.key === 'Enter') {
                e.preventDefault();
                handleAddMsg(tab === 'notes' ? 'note' : tab === 'reminders' ? 'reminder' : 'question');
              }
            }}
          ...
          />
          <button 
            onClick={() => handleAddMsg(tab === 'notes' ? 'note' : tab === 'reminders' ? 'reminder' : 'question')}
